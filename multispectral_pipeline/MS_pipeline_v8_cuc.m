...
%% ============ PLANT SEGMENTATION + NDVI-BASED MASK FILTERING ============
...
% Remove small speckles from raw segmentation
Mask = bwareaopen(Mask, minRegionSize);

% Apply NDVI-based filtering to the mask (lower & upper bounds)
Mask(NDVI < ndviLowerThresh) = 0;
Mask(NDVI > ndviUpperThresh) = 0;

% Tidy up again after NDVI filtering
Mask = bwareaopen(Mask, minRegionSize);

% Create NDVI masked image and compute stats
