"""
camera_control_and_data_acquisition.py
---------------------------------------
...
def save_image_NIR(image_data, images_path, frame_count):
    ...
    with open(os.path.join(images_path, image_name), "wb") as f:
        f.write(image_data.tobytes())  # save as raw binary data
    print("Saved NIR image:", image_name)
...
def acquire_images(device, source0, source1, images_path):
    ...
    while frame_count < max_frames:
        ...
        if recieived_frames_for_source == 0:
            ...
            # retrieve and discard Source1 buffer to prevent queue overflow
            ...
            if not result.IsOK():
                print(f"source{recieived_frames_for_source} {result.GetCodeString()}")
            # retrieve and discard Source0 buffer to prevent queue overflow
        ...
        if result.IsOK():
            if operational_result.IsOK():
                ...
                print(f"{doodle[doodle_index]} ImgID: {pvbuffer.GetBlockID():03d}")
                ...
                if payload_type == eb.PvPayloadTypeImage:
                    ...
                    print(f" Raw Data: {pvbuffer.GetRawData().GetPayloadLength()} bytes")
                elif payload_type == eb.PvPayloadTypeMultiPart:
                    print(f" Multi Part with {pvbuffer.GetMultiPartContainer().GetPartCount()} parts")
                    ...
                    if eb.PvDecompressionFilter.IsCompressed(pvbuffer):
                        ...
                        if result.IsOK():
                            ...
                            if result.IsOK():
                                ...
                                print(" Could not decompress (Pleora compressed)")
                                errors += 1
                        else:
            ...
            # Queue the buffer back to the correct stream
            if recieived_frames_for_source == 0:
    ...
    for i in range(max_frames):
        ...
        if result_nir.IsOK() and operational_result_nir.IsOK():
            ...
            print(f"source1 {result_nir.GetCodeString()}")
    ...
    if connection_ID:
        ...
        if device:
            ...
            if stream1 and stream2:
                ...
                acquire_images(device, stream1, stream2, images_path=IMAGES_PATH)
                buffer_list1.clear()
                buffer_list2.clear()
